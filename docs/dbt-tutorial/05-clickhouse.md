...
| **View (FINAL)** | 중복 제거 래퍼 | X | `SELECT ... FROM _distributed FINAL`을 감싼 뷰. 중복 제거까지 보장 |

!!! tip "View (FINAL)이란?"
    ClickHouse의 `ReplicatedReplacingMergeTree` 등은 데이터 merge 전까지 중복 행이 일시적으로 존재할 수 있습니다. `FINAL`을 붙이면 이 중복을 제거한 최종 결과를 보장합니다.
    ```sql
    -- View 정의 예시
  ...
  <div class="flow-panels" style="flex-direction:column;">
    ...
    <div style="display:flex; gap:12px; padding:0 12px 12px;">
      <div style="flex:1; background:#181825; border-radius:8px; padding:10px;" id="ch-ld-shard1-box">
        <div class="flow-panel-label" style="background:#a6e3a133;color:#a6e3a1;">Node 1 (Shard 1 · 짝수)</div>
        <div id="ch-ld-shard1-area"></div>
      </div>
      <div style="flex:1; background:#181825; border-radius:8px; padding:10px;" id="ch-ld-shard2-box">
        <div class="flow-panel-label" style="background:#f9e2af33;color:#f9e2af;">Node 3 (Shard 2 · 홀수)</div>
...
<div class="flow-demo" id="ch-mv-compare" markdown="0">
  ...
  <div class="flow-type-bar">
    <button class="mat-btn active" data-type="simple" onclick="chMVSelectCase('simple')">케이스 1: 단순 집계 (DAU)</button>
    <button class="mat-btn" data-type="complex" onclick="chMVSelectCase('complex')">케이스 2: 복잡한 JOIN 연산</button>
  </div>
  <div id="ch-mv-case-desc" style="padding:8px 12px;font-size:11px;color:#6c7086;background:#181825;"></div>
  <div class="flow-panels">
    <div class="flow-panel" style="border-right: 2px solid #45475a;">
      <div class="flow-panel-label" style="background:#89b4fa33;color:#89b4fa;" id="ch-mv-source-label">Source</div>
      <div id="ch-mv-source-area"></div>
      <div id="ch-mv-source2-area"></div>
    </div>
    <div class="flow-panel" style="flex:1.5;">
      <div style="display:flex; gap:8px;">
        ...
        <div style="flex:1;">
          ...
          <div style="font-size:9px;color:#6c7086;margin-bottom:4px;">주기적 전체 재계산 · JOIN/FINAL 가능</div>
